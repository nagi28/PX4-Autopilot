...
>
> **なぜ EKF2 の `ax/ay/az` を使わないのか:** EKF2 も加速度を状態量として
> 推定していません。あのフィールドは「加速度計の Δv をバイアス補正・NED 回転・
> 重力除去して平均化しただけ」の、フィルタを通っていない生値です
> （[03 章の「3 つの出力の出自は違う」](03_sensing_estimation.md#3-つの出力の出自は違う)）。
> D 項に直接入れるとノイズで制御が荒れるため、
> 制御器が自分で「フィルタ済み速度の微分」を作るほうが素性が良いわけです。
>
